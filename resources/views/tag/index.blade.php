                        ...
                        <ul class="list-group">
                            @foreach($tags as $tag)
                                <li class="list-group-item">
                                    <a href="/tag/{{$tag->id}}"><span style="font-size: 130%;" class="mr-2 badge badge-{{ $tag->style }}">{{ $tag->name }}</span></a>
                                        ...
                                        <form style="display: inline;" action="/tag/{{ $tag->id }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <input onclick="return confirm('Are Youe sure ;You Want to delete?');" class="btn btn-outline-danger btn-sm ml-2" type="submit" value="Delete">
                                        </form>
